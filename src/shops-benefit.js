    ...
    document.querySelectorAll('#type > option').forEach(title => {
        ...
        .then(rep => {
            ...
            for (let i in data) {
                ...
                await temp += `<tr class="${title.id} disable">`;
                for (let j in data[i].c) {
                    if (j == 2 && (data[i].c[3].v).indexOf('IG') != -1) {
                        await temp += '<td>' + `<iframe src="https://www.instagram.com/${data[i].c[j]?.v}/embed" scrolling="no" frameborder="0"></iframe>` + '</td>';
                    } else if (j == 2 || j > 4) {
                        await temp += '<td>' + data[i].c[j]?.v || 'N/A' + '</td>';
                    };
                };
                await temp += '</tr>';
            };
