            ...
            for (let i in data) {
                ...
                for (let j in data[i].c) {
                    if (j == 2 && (data[i].c[3].v).indexOf('IG') != -1) {
                        temp += '<td>' + `<span><a href="https://instagram.com/${data[i].c[j]?.v}" target="_blank">@${data[i].c[j]?.v}</a></span><br><br><iframe src="https://www.instagram.com/${data[i].c[j]?.v}/embed" scrolling="no" frameborder="0"></iframe>` + '</td>';
                    } else if (j == 2 || j > 4) {
